...
extension CreateViewController {
        ...
        listener = collectionRef.addSnapshotListener({ querySnapshot, error in
           ...
           // Process retrieved documents
            ...
            DispatchQueue.main.async {
                self.createView.categoryPicker.reloadAllComponents()
    ...
    func addNewTransaction(amount: Double, category: Category, timestamp: Date, description: String, photoURL: URL?) {
        ...
        do {
            // Add a new transaction document under the user's document
            try collectionRef.addDocument(from: newTransaction) { error in
                if let error = error {
                    self.hideActivityIndicator()
                    self.presentAlertWithTitle("Error", message: "Error adding transaction: \(error)")
                } else {
                    self.hideActivityIndicator()
                    self.clearInput()
                    self.presentAlertWithTitle("Success", message: "Transaction added successfully!")
                }
            }
        } catch {
                ...
                let uploadTask = imageRef.putData(jpegData, completion: {(metadata, error) in
                    if error == nil{
                        imageRef.downloadURL(completion: {(url, error) in
                            if error == nil{
                                photoURL = url
                                guard let amountText = self.createView.amountField.text,
                                   let amountValue = Double(amountText) else { return }
                                
                                self.addNewTransaction(amount: amountValue, category: self.selectedCategory!, timestamp: self.selectedDate!, description: self.createView.descriptionTextView.text, photoURL: photoURL)
                            }
                        })
                    } else {
                        self.hideActivityIndicator()
