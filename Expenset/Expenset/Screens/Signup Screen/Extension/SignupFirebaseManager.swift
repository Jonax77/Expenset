...
extension SignupViewController {
           ...
           let password = signupView.textFieldPassword.text {
            ...
            Auth.auth().createUser(withEmail: email, password: password) { [weak self] result, error in
                ...
                if error == nil, let uid = result?.user.uid {
                    ...
                    self.createNewUserInFirestore(uid: uid, email: email, displayName: name, photoURL: photoURL)
                } else {
    ...
    func createNewUserInFirestore(uid: String, email: String, displayName: String, photoURL: URL?) {
